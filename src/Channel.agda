...
data ValidChannelRef : (G : SCtx) (b : Bool) (s : STypeF SType) → Set where
  ...
  here-neg : ∀ {s s'} {G : SCtx}
    → (ina-G : Inactive G)
    → dualF s ≲' s'
    → ValidChannelRef (just (s , NEG) ∷ G) false s'
  there : ∀ {b s} {G : SCtx}
...
vcr-match-sr {just (recv t s , POSNEG) ∷ G} {just _ ∷ G₁} {just _ ∷ G₂} (ss-posneg ss) (here-pos ina-G₁ _) (here-neg ina-G₂ _) =
  just ({!!} , {!!} , just (_ , POSNEG) ∷ G , just (_ , POS) ∷ G₁ , just (_ , NEG) ∷ G₂ , ss-posneg ss ,  here-pos ina-G₁ {!!} , here-neg ina-G₂ {!!} )
vcr-match-sr {just (send t s , POSNEG) ∷ G} {just _ ∷ G₁} {just _ ∷ G₂} (ss-negpos ss) (here-neg ina-G₁ _) (here-pos ina-G₂ _) =
  just ({!!} , {!!} , just (_ , POSNEG) ∷ G , just (_ , NEG) ∷ G₁ , just (_ , POS) ∷ G₂ , ss-negpos ss , here-neg ina-G₁ {!!} , here-pos ina-G₂ {!!})
-}
...
vcr-match-2-nsb : ∀ {G G₁ G₂ G₁₁ G₁₂ b₁ b₂ m₁ m₂ alti alte}
  ...
  → Maybe (Σ (m₁ ≤ m₂) λ { p → 
          ...
          ValidChannelRef G₁₂' b₂ (SType.force (alte (inject≤ lab p)))})
...
vcr-match-2-nsb (ssplit2 ss1 ss2) (here-neg ina-G x) (here-pos ina-G₁ x₁) lab = {!!}
